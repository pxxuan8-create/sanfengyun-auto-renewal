...
## 运行机制（GitHub Actions 每 2 天一次 + 随机时分秒）

- **触发**：`schedule: 0 0 */2 * *`（UTC 0:00 = 北京 8:00，每 2 天一次）
- **随机时分秒**：GitHub cron 不支持随机，workflow 里加了 `RANDOM % 14400`（0~4 小时）随机延迟，**每次实际执行时间点（时分秒）都不同**，规避"固定时间=定时任务"的人工检测
- 手动触发：仓库 Actions 页 → 左侧 Workflow → "三丰云自动延期" → **Run workflow** 按钮
- **每次运行完整闭环**：扫描 → 提交 → 发「已提交」邮件 → 每 5 分钟轮询审核（最多约 3 小时）→ 审核通过发「延期成功」/ 失败发「延期失败」
- 提交/审核逻辑沿用原仓库的持续循环判断（`--once` 模式单次跑完整个流程）；原仓库每 60 秒醒一次的常驻 `run_loop` 模式**保留未删**，需要自建服务器时可改用该模式
...
【三丰云延期检查】2026-09-09 08:23:45     ← 本轮无延期操作（未到时间/审核中/未开通）
```
...
| `python sanfengyun_renewal.py --once` | 扫描 → **可以延期的都完整提交一次** → 每 5 分钟轮询审核 → 发结果邮件 | Actions 每次触发跑的就是这个 |
