...
| ③ 提交 | 三丰云后台填入博文 URL + 上传博文截图，提交延期表单 | 提交成功 → 冷却 4h 等审核 |
| ④ 通知 | **每次运行结束发一封汇总邮件**（免登录确认结果） | 标题一眼辨成败：`【三丰云延期成功/失败/检查】+ 北京时间` |

**每个产品独立判断**，到期的才处理，未到时间的只记录。

---

## 运行机制（GitHub Actions 每 2 天一次）

- **触发**：`schedule: 0 0 */2 * *`（UTC 0:00 = 北京 8:00，每 2 天一次）
- **随机时间**：GitHub cron 不支持随机，workflow 里加了 `RANDOM % 14400`（0~4 小时）随机延迟，**每次实际执行时间点都不同**，规避"固定时间=定时任务"的人工检测
- 手动触发：仓库 Actions 页 → 左侧 Workflow → "三丰云自动延期" → **Run workflow** 按钮
- 每次运行结束 → 脚本把各产品结果汇总 → 发一封 SMTP 邮件
...
├── .github/workflows/sanfengyun_renew.yml   ⭐ GitHub Actions 定时任务定义
├── README.md                 📖 本文件（给人看）
├── AI_SPEC.yaml              🤖 机器可读规约（给 AI 看）
├── sanfengyun_renewal.py     ⭐ 主入口（--once 单次执行）
...
├── email_notify.py           ✅ SMTP 邮件通知（替代原钉钉）
...
每次运行结束，脚本把各产品结果汇总成**一封**邮件发送：

**标题**：
```
【三丰云延期成功】2026-09-09 08:23:45     ← 至少一个成功且无失败
【三丰云延期失败】2026-09-09 08:23:45     ← 有任一失败
【三丰云延期检查】2026-09-09 08:23:45     ← 本次无延期操作（未到时间/审核中）
```
...
==============================
(到期时间以三丰云控制台实际显示为准；延期需工作时间审核)
```

> 📌 **延期天数来源**：免费云服务器 +5 天/次、免费虚拟主机 +30 天/次（三丰云官方规则）。到期时间 = 扫描到的当前到期时间 + 延期天数。
> 📌 **只发一封汇总邮件**，过程性通知（扫描、倒计时等）只写日志，不单独发邮件轰炸你。
...
| `python sanfengyun_renewal.py --once` | 扫描 → **可以延期的都完整提交一次** | Actions 每次触发跑的就是这个 |
| `python sanfengyun_renewal.py` | 持续循环模式（7×24 常驻，需自己服务器） | 不用 Actions 时自建服务器用 |

> 邮件通知在 `--test` / `--once` 结束后都会发（test 模式发"检查/失败"类摘要，可用来验证邮件通道）。
